...
type QuestionItem = Awaited<ReturnType<typeof questionRepository.findAllWithScores>>["items"][number];
...
export const handler: Handlers<undefined, State> = {
  async GET(req, ctx) {
    ...
    const result = await questionRepository.findAllWithScores(cursor, ctx.state.sessionUser?.id ?? null);
