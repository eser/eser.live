...
import type { QuestionVote, QuestionVotePartial } from "../question-vote/types.ts";
import { userSchema } from "../user/schema.ts";
import { questionSchema } from "./schema.ts";
import type { Question, QuestionPartial, QuestionWithScores } from "./types.ts";

export class QuestionRepository {
  async findAll(cursor: Cursor) {
    const filterConditions = cursor.offset === ""
      ? isNull(questionSchema.deletedAt)
      : and(gt(questionSchema.id, cursor.offset), isNull(questionSchema.deletedAt));

    const query = db.query.questionSchema.findMany({
  ...
  async findAllByUserId(cursor: Cursor, userId: string) {
    const filterConditions = cursor.offset === ""
      ? and(eq(questionSchema.userId, userId), isNull(questionSchema.deletedAt))
      : and(
  ...
  async findAllWithScores(cursor: Cursor, viewingUserId: string | null) {
    ...
    const query = db
      .select({
        ...
        scoreSumTotal: sql<number>`CAST(COALESCE(SUM(${questionVoteSchema.score}), 0) AS INT)`.as("total_score_sum"),
        scoreSumUser: scoreSumUserCalculation.as("user_score_sum"),
      })
      .from(questionSchema)
      .leftJoin(userSchema, and(eq(userSchema.id, questionSchema.userId), isNull(userSchema.deletedAt)))
      .leftJoin(questionVoteSchema, eq(questionVoteSchema.questionId, questionSchema.id))
      .where(
        and(
          cursor.offset !== "" ? gt(questionSchema.id, cursor.offset) : undefined,
          eq(questionSchema.isHidden, false),
          isNull(questionSchema.deletedAt),
        ),
      )
      .groupBy(questionSchema.id, userSchema.id)
  ...
  async findAllByUserIdWithScores(cursor: Cursor, userId: string, viewingUserId: string | null) {
    const scoreSumUserCalculation = viewingUserId !== null
      ? sql<
        number
      >`CAST(SUM(CASE WHEN ${questionVoteSchema.userId} = ${viewingUserId} THEN ${questionVoteSchema.score} ELSE 0 END) AS INT)`
  ...
  async findById(id: string) {
    const result: Question | undefined = await db.query.questionSchema.findFirst({
  ...
  async create(question: QuestionPartial) {
    const result: Question | undefined = (await db.insert(questionSchema).values(question).returning()).at(0);
