...
import { type Session, type SessionPartial } from "./types.ts";
...
export class SessionRepository {
  ...
  async create(session: SessionPartial) {
    const result: Session | undefined = (await db.insert(sessionSchema)
      .values(session)
      .returning()).at(0);
  ...
  async update(id: string, session: Partial<SessionPartial>) {
    const result: Session | undefined = (await db.update(sessionSchema)
