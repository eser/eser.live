...
import type { EventAttendance, EventAttendancePartial } from "../event-attendance/types.ts";
...
import type { Event, EventPartial, EventWithStats } from "./types.ts";

export class EventRepository {
  async findAll(cursor: Cursor) {
    const filterConditions = and(
      cursor.offset !== "" ? gt(eventSchema.timeStart, new Date(Number(cursor.offset))) : undefined,
      isNull(eventSchema.deletedAt),
    );

    const query = await db.query.eventSchema.findMany({
  ...
  async findAllAttendancesByProfileId(cursor: Cursor, profileId: string) {
    ...
    const query = db.query.eventSchema.findMany({
      where: filterConditions,
      with: {
        attendances: {
  ...
  async upsertAttendance(eventAttendance: EventAttendancePartial) {
    const result: EventAttendance | undefined = (
      await db
        .insert(eventAttendanceSchema)
        .values(eventAttendance)
        .onConflictDoUpdate({
          target: [eventAttendanceSchema.eventId, eventAttendanceSchema.profileId],
          set: eventAttendance,
        })
        .returning()
    ).at(0);
  ...
  async findAllWithStats(cursor: Cursor, until: Date | null, viewingProfileId: string | null) {
    ...
    const query = db
      .select({
        ...
        statSumTotal: sql<number>`CAST(COALESCE(COUNT(${eventAttendanceSchema.id}), 0) AS INT)`.as("total_stat_sum"),
        statSumProfile: statSumProfileCalculation.as("profile_stat_sum"),
      })
      .from(eventSchema)
      .leftJoin(
        eventSeriesSchema,
        and(eq(eventSeriesSchema.id, eventSchema.seriesId), isNull(eventSeriesSchema.deletedAt)),
      )
      .leftJoin(
        eventAttendanceSchema,
        and(eq(eventAttendanceSchema.eventId, eventSchema.id), isNull(eventAttendanceSchema.deletedAt)),
      )
      .leftJoin(
        profileSchema,
        and(eq(profileSchema.id, eventAttendanceSchema.profileId), isNull(profileSchema.deletedAt)),
      )
      .where(
        and(
          cursor.offset !== "" ? gt(eventSchema.timeStart, new Date(Number(cursor.offset))) : undefined,
      ...
      .groupBy(eventSchema.id, eventSeriesSchema.id)
  ...
  async findById(id: string) {
    const result: Event | undefined = await db.query.eventSchema.findFirst({
  ...
  async create(event: EventPartial) {
    const result: Event | undefined = (await db.insert(eventSchema).values(event).returning()).at(0);
