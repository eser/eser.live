...
Deno.test("[plugins] getGitHubUser()", async (test) => {
  await test.step("rejects on error message", async () => {
    ...
    await assert.assertRejects(async () => await getGitHubUser(ulid.ulid()), BadRequestError, message);
    fetchStub.restore();
  });

  await test.step("resolves to a GitHub user object", async () => {
    const body = { login: ulid.ulid(), email: ulid.ulid() };
    const fetchStub = mock.stub(globalThis, "fetch", mock.returnsNext([Promise.resolve(Response.json(body))]));
