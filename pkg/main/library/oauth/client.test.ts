...
Deno.test({
  name: "getSuccessUrl() returns `success_url` URL parameter, if defined",
  fn: () => {
    assert.assertEquals(
      getSuccessUrl(
        new Request(
...
Deno.test({
  name: "getSuccessUrl() returns referer header of same origin, if defined",
  fn: () => {
...
Deno.test({
  name: "getSuccessUrl() returns root path if referer is of different origin",
  fn: () => {
    assert.assertEquals(
      getSuccessUrl(
...
Deno.test({
  name: "getSuccessUrl() returns root path by default",
  fn: () => {
    assert.assertEquals(
...
Deno.test({
  ...
  fn: async () => {
    ...
    assert.assertEquals(setCookie.name, OAUTH_COOKIE_NAME);
